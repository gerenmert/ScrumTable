...
// Aplication Model (Uygulama Modeli)
var workspaces = {}
var tasks = {}
...
$(function (){
	...
	// Task Actions (Görev Hareketeleri)
	...
	// Add/Edit Task Bindings (Ekle/Düzenle Görev Bağlantıları)
	$("#btnAddTask").click(onClickBtnAddTask);
	$('#btnTaskCancel').click(hideModals);
	$("#btnTaskOk").click(onClickBtnTaskOk);			

	//Preview Task Binding (Görev Bağlantılarını Önizle)
	$("#btnTaskPreviewOk").click(hideModals);

	// Export Bindings (Dışarıya Gönderme Bağlandıları)
	$("#export").click(onClickExport);
	$("#btnExportClose").click(hideModals);

	// Import Bindings (İçeriye Getirme Bağlantıları)
	...
	// Workspaces Bindings (Çalışma Alanı Bağlantıları)
...
/* General Functions (Genel Fonksiyonlar) */
...
/* Drag Events (Sürükleme Olayları) */
...
function onClickRemoveWorkspace(){
	if(confirm(message("confirm_workspace_remove",workspaces[currentWorkspace]))){
		//Remove Old Workspace (Eski Çalışma Alanını Kaldır)
